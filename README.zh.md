...
- [zhubaohi/dsh-gpu-pulse](https://github.com/zhubaohi/dsh-gpu-pulse) — DSH Web 界面内置的悬浮 GPU 监控：基于 nvidia-smi 实时显示每张 GPU 的利用率、显存、温度、功耗与风扇（支持多卡），带历史走势图，可折叠为一行胶囊；驱动支持时还列出显存占用最高的进程；无 NVIDIA 驱动的机器会退化为 n/a 胶囊。
